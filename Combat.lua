...
Page5.Toggle({
	...
	Callback = function(value)
		...
		if value == true then
			warehouse = Instance.new("Part")
			warehouse.CanCollide = false
			warehouse.Anchored = true
			warehouse.Transparency = 1
			CollectionService:AddTag(warehouse, "Warehouse")
			CollectionService:AddTag(warehouse, "Interactable")
			warehouse.Parent = workspace
			warehouse.CFrame = Player.Character:FindFirstChild("HumanoidRootPart").CFrame
		else
			if warehouse then 
				warehouse.Position = warehouse.Position + Vector3.new(0,100,0)
				task.wait(2)
				warehouse:Destroy() 
				warehouse = nil
			end
